...
class ConnectionNotifier extends StateNotifier<ConnectionServiceState> {
  ...
  ConnectionNotifier(this._api, this._profile)
      : super(const ConnectionServiceState());

  void _setStateSafe(
      ConnectionServiceState Function(ConnectionServiceState current) updater) {
    if (!mounted) return;
    state = updater(state);
  }
  ...
  Future<InviteCodeInfo?> generateInviteCode() async {
    ...
    _setStateSafe((s) => s.copyWith(isLoading: true, error: null));

    try {
      AppLogger.api('POST', '/api/connections/generate-code');
      final response =
          await _api.post('/api/connections/generate-code', body: {});
      AppLogger.api('POST', '/api/connections/generate-code',
      ...
      if (response.success && response.data != null) {
        ...
        _setStateSafe(
            (s) => s.copyWith(isLoading: false, activeInviteCode: inviteCode));
        return inviteCode;
      } else {
        // If server fails, generate a mock code for demo purposes
        if (response.statusCode == 0) {
          // Network error - use mock data
          AppLogger.warning('Network error (statusCode=0), using mock code',
              tag: 'CONNECT');
          ...
          _setStateSafe((s) =>
              s.copyWith(isLoading: false, activeInviteCode: mockInvite));
          return mockInvite;
        }
        AppLogger.error('Failed to generate code: ${response.error}',
            tag: 'CONNECT');
        _setStateSafe((s) => s.copyWith(
              isLoading: false,
      ...
      AppLogger.error('Exception generating code: $e',
          tag: 'CONNECT', stackTrace: stackTrace);
      _setStateSafe((s) => s.copyWith(isLoading: false, error: 'Error: $e'));
      return null;
    }
  }
  ...
  Future<void> loadConnections() async {
    ...
    try {
      ...
      if (response.success && response.data != null) {
        ...
        _setStateSafe(
            (s) => s.copyWith(isLoading: false, connections: connectionsList));
      } else {
        _setStateSafe((s) => s.copyWith(isLoading: false));
      }
    } catch (e) {
      _setStateSafe((s) => s.copyWith(isLoading: false, error: 'Error: $e'));
    }
  ...
  Future<void> loadMyElderly() async {
    ...
    try {
      ...
      if (response.success && response.data != null) {
        ...
        _setStateSafe(
            (s) => s.copyWith(isLoading: false, linkedElderly: elderlyList));
      } else {
        _setStateSafe((s) => s.copyWith(isLoading: false));
      }
    } catch (e) {
  ...
  Future<void> loadMyCaregivers() async {
    ...
    try {
      ...
      if (response.success && response.data != null) {
        ...
        _setStateSafe((s) =>
            s.copyWith(isLoading: false, linkedCaregivers: caregiverList));
      } else {
        _setStateSafe((s) => s.copyWith(isLoading: false));
      }
    } catch (e) {
    ...
    try {
      ...
      if (response.success) {
        ...
        _setStateSafe((s) => s.copyWith(
              isLoading: false,
              connections:
                  s.connections.where((c) => c.id != connectionId).toList(),
            ));
  ...
  /// Clear error
  void clearError() {
    _setStateSafe((s) => s.copyWith(error: null));
  }
}
...
final connectionProvider =
    StateNotifierProvider<ConnectionNotifier, ConnectionServiceState>((ref) {
