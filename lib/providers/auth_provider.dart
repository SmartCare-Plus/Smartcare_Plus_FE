...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Future<bool?> signInWithGoogle() async {
    ...
    try {
      ...
      final isConfigError = rawMessage.contains('10') ||
          rawMessage.contains('developer_error') ||
          rawMessage.contains('12500') ||
          rawMessage.contains('api exception');

      state = state.copyWith(
        isLoading: false,
        error: isConfigError
            ? 'Google Sign-In is not configured for this app build (SHA/client ID mismatch). Please update Firebase Android OAuth settings.'
  ...
  Future<bool> updateProfile({
    ...
    try {
      ...
      if (guardianProfile != null) {
        updates['guardian_profile'] = guardianProfile;
      }
